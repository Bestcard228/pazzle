...
# MEDIUM chains two PATTERNs, so it has an intermediate state that must be hit.
static func is_two_stage(difficulty: int) -> bool:
...
# MEDIUM chains a second PATTERN onto the first, starting from its final Draw -- that Draw
# is shared, it is both the previous pattern's END and the new pattern's opening.
#
#   D S D  +  D D S   ->   D S D D S      (the middle D is the seam)
#
# Only a PATTERN ending in Draw can be continued, so [D,D,S] is a terminal branch and
# never appears first. That leaves 2 openers x 3 continuations x 2 prefixes = 12 chains of
# 5 or 6 turns.
static func get_medium_sequences() -> Array:
	var sequences: Array = []

	for prefix in EASY_PREFIXES:
		for first in EASY_PATTERNS:
			if first[first.size() - 1] != 1:
				continue # a pattern ending in Skip has no Draw to continue from
			for second in EASY_PATTERNS:
				var sequence: Array[int] = []
				for action in prefix:
					sequence.append(int(action))
				for action in first:
					sequence.append(int(action))
				# Skip the second pattern's opening Draw: it is the seam we already placed
				for i in range(1, second.size()):
					sequence.append(int(second[i]))
				sequences.append(sequence)

	return sequences


static func get_sequences_for(difficulty: int) -> Array:
	return get_medium_sequences() if is_two_stage(difficulty) else get_easy_sequences()


# The turn the first PATTERN ends on -- the seam where the second pattern picks up. The
# geometry alive at this turn is the "leftover" the second half builds from.
static func get_stage_boundary_turn(sequence: Array) -> int:
	return sequence.size() - EASY_PATTERN_DEPTH


static func _take_next_sequence(difficulty: int) -> Array[int]:
	var bag := _medium_sequence_bag if is_two_stage(difficulty) else _easy_sequence_bag

	if bag.is_empty():
		bag.assign(get_sequences_for(difficulty))
		bag.shuffle()
...
static func generate_puzzle(
	...
	for attempt in range(max_attempts):
		...
		# 2. Every drawn shape must matter. In a two-stage puzzle the first half is wiped
		# before the final target is read, so its shapes are judged against the seam
		# state instead -- the leftovers the second pattern builds from.
		if is_two_stage(difficulty):
			var boundary := get_stage_boundary_turn(easy_sequence)

			if not PuzzleValidator.validate_stage_contributions(sol, puzzle_board, 0, boundary):
				continue

			if not PuzzleValidator.validate_stage_contributions(sol, puzzle_board, boundary + 1, max_turns - 1):
				continue

			# The seam must be a real intermediate goal, not a near-empty one
			var seam := PuzzleSimulator.simulate_up_to_turn(sol, puzzle_board, boundary)
			if seam.segments.size() < 2:
				continue

			# ...and it must differ from the final target, or the second half is busywork
			if seam.is_equivalent_to(target):
				continue
		...
		# 5. Timing has to matter. Skipped for two-stage puzzles: that check asks whether
		...
		# enforce.
		if not is_two_stage(difficulty):
		...
		if is_two_stage(difficulty):
			p_data.stage_boundary_turn = get_stage_boundary_turn(easy_sequence)
			p_data.checkpoint_geometry = PuzzleSimulator.simulate_up_to_turn(
				sol,
				puzzle_board,
				p_data.stage_boundary_turn
			)
