...
func _connect_to_main_menu() -> void:
    var main_menu = get_tree().root.get_node("Main/MainMenu", true, false)
    if main_menu:
...
func _on_settings_applied(language: String, mode: String, color_mode: bool, sound_enabled: bool) -> void:
    if language != current_locale:
        ...
        get_tree().root.get_node("Main/MainMenu").settings_applied.emit(language, mode, color_mode, sound_enabled)
