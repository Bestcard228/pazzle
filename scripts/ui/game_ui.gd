...
# A chained puzzle reveals one goal at a time: the first pattern's target, and only once
# that is reached the next one, which builds on the leftovers still on the board.
func _refresh_stage_targets() -> void:
	...
	# The card beside it shows the stage already banked, so the player can see what the
	# current goal is being built on top of.
	var show_previous := current_puzzle.is_multi_stage() and current_stage > 0
	checkpoint_display.visible = show_previous
	if show_previous:
		checkpoint_display.set_target(current_puzzle.get_stage_target(current_stage - 1), current_puzzle.board_definition)
